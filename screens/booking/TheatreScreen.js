...
const TheaterScreen = ({ route, navigation }) => {
  ...
  const subscribe = async() => {
    const response = await fetch("http://192.168.0.106:8000/pay", {
      method: "POST",
      body: JSON.stringify({
    ...
    numColumns={7}
        ...
        renderItem={({ item }) => (
          ...
          >
            {
              seats.includes(item) ? (
                <Text style={{ backgroundColor: "#ffc40c", padding: 8,color:"#fff" }}>{item}</Text>
              )
              :
              occupied.includes(item) ? (
                <Text style={{ backgroundColor: "#989898", padding: 8,color:"#fff" }}>{item}</Text>
              )
              :
              (
                <Text style={{ padding: 8,color:"#fff" }}>{item}</Text>
              )
            }
          </Pressable>
        )}
